...
import com.au.data2ml.uaa.config.KafkaProperties;
...
class UserauthServiceKafkaResourceIT {
    ...
    void setup() {
        ...
        UserauthServiceKafkaResource kafkaResource = new UserauthServiceKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/userauth-service-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/userauth-service-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
